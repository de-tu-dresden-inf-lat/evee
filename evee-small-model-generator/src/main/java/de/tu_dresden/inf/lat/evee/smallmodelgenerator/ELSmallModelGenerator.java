...
public class ELSmallModelGenerator implements IOWLModelGenerator, IHasProgressTracker {
    ...
    public void setOntology(OWLOntology ont) {
        ...
        try {
            ...
            ont.getAxioms(Imports.INCLUDED).forEach(ax -> man.addAxiom(ontology, ax));
        } catch (OWLOntologyCreationException e1) {
            logger.error("ontology could not be created");
            e1.printStackTrace();
            return;
        }
        ...
        if (!hasSuccessor) {
            ...
            if (!this.treeModel) {
                for (OWLNamedIndividual ind : this.ont.getIndividualsInSignature()) {
                    ...
                    toAdd.forEach(ax -> man.addAxiom(ont, ax));
                    res.flush();
                    ...
                    if (isConsisted && !avoidedEntHolds) {
                        ...
                        toAdd.forEach(ax -> man.removeAxiom(ont, ax));
