...
//        ontology = ALCHTBoxFilter.filteredCopy(owlOntology, ontologyManager);
    ...
    private OWLOntology justify(OWLOntology ontology, OWLAxiom axiom) {
        ...
        OWLOntology resultOntology = OWLTools.createOntology(explanation);
        if (resultOntology == null) {
            logger.warn("Could not create ontology from explanation, returning original ontology as fallback.");
            return ontology;
        }
    ...
    private static OWLOntology getEmptyOntology(OWLOntology baseOntology) {
        ...
        OWLOntology result= OWLTools.createOntology(declarations);
        if(result==null) {
            logger.error("Could not create empty ontology from input ontology");
