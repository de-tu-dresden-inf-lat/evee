 ...
 *
 */
public class OWLSuperClassExpressionSplitter implements OWLClassExpressionVisitorEx<Collection<OWLSubClassOfAxiom>> {
	...
	public OWLSuperClassExpressionSplitter(OWLClassExpression sub, Set<OWLEntity> sig, ConceptNameGenerator A) {
		this.sub = sub;
		this.sig = sig;
		this.A = A;
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectIntersectionOf ce) {
		ce.getOperands().stream().filter(e -> !e.isOWLThing()).map(e -> OWLTools.odf.getOWLSubClassOfAxiom(sub, e))
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectUnionOf ce) {
		if (!ce.getOperands().stream().anyMatch(e -> OWLTools.isAuxiliary(e, sig))) {
			newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(sub,
					OWLTools.odf.getOWLObjectUnionOf(ce.getOperands().stream().map(e -> {
						OWLClass ar = A.next();
						newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(ar, e));
						return ar;
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectComplementOf ce) {
		if (!ce.getOperand().isOWLNothing() && !OWLTools.isAuxiliary(ce.getOperand(), sig)) {
			OWLClass ac = A.next();
			newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(sub, OWLTools.odf.getOWLObjectComplementOf(ac)));
			newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(ce.getOperand(), ac));
		}
		return nullOrNewAxioms();
	}
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectSomeValuesFrom ce) {
		if (!ce.getFiller().isOWLThing() && !OWLTools.isAuxiliary(ce.getFiller(), sig)) {
			OWLClass ac = A.next();
			newAxioms.add(
					OWLTools.odf.getOWLSubClassOfAxiom(sub, OWLTools.odf.getOWLObjectSomeValuesFrom(ce.getProperty(), ac)));
			newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(ac, ce.getFiller()));
		}
		return nullOrNewAxioms();
	}
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectAllValuesFrom ce) {
		if (!ce.getFiller().isOWLThing() && !OWLTools.isAuxiliary(ce.getFiller(), sig)) {
			OWLClass ac = A.next();
			newAxioms.add(
					OWLTools.odf.getOWLSubClassOfAxiom(sub, OWLTools.odf.getOWLObjectAllValuesFrom(ce.getProperty(), ac)));
			newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(ac, ce.getFiller()));
		}
		return nullOrNewAxioms();
	}
	...
	public Collection<OWLSubClassOfAxiom> visit(OWLObjectHasValue ce) {
		OWLClass ac = A.next();
		newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(sub, OWLTools.odf.getOWLObjectSomeValuesFrom(ce.getProperty(), ac)));
		newAxioms.add(OWLTools.odf.getOWLSubClassOfAxiom(ac, OWLTools.odf.getOWLObjectOneOf(ce.getFiller())));
		return nullOrNewAxioms();
	}
