...
public class OWLSubClassExpressionUnfolder implements OWLClassExpressionVisitorEx<OWLClassExpression> {
	...
	public OWLClassExpression visit(OWLClass ce) {
		if (OWLTools.isAuxiliary(ce, sig)) {
			...
			List<OWLClassExpression> def = axs.stream().filter(ax -> ax.getSuperClass().equals(ce))
					.map(ax -> ax.getSubClass()).map(s -> s.accept(this)).collect(Collectors.toList());
			...
			default:
					return OWLTools.odf.getOWLObjectUnionOf(def.toArray(OWLClassExpression[]::new));
				}
		} else {
			return ce;
		}
