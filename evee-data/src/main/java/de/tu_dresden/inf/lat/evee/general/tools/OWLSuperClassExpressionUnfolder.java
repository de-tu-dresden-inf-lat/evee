...
public class OWLSuperClassExpressionUnfolder implements OWLClassExpressionVisitorEx<OWLClassExpression> {
	...
	public OWLClassExpression visit(OWLClass ce) {
		if (OWLTools.isAuxiliary(ce, sig)) {
			...
			List<OWLClassExpression> def = axs.stream().filter(ax -> ax.getSubClass().equals(ce))
					.map(ax -> ax.getSuperClass()).map(s -> s.accept(this)).collect(Collectors.toList());
			...
			default:
				return OWLTools.odf.getOWLObjectIntersectionOf(def.toArray(OWLClassExpression[]::new));
			}
		} else {
			return ce;
		}
	...
	public OWLClassExpression visit(OWLObjectIntersectionOf ce) {
		List<OWLClassExpression> op = visit(ce.getOperands()).collect(Collectors.toList());
		if (op.contains(null)) {
			return null;
		}
		return OWLTools.odf.getOWLObjectIntersectionOf(op.toArray(OWLClassExpression[]::new));
	}
	...
	public OWLClassExpression visit(OWLObjectUnionOf ce) {
		...
		return OWLTools.odf.getOWLObjectUnionOf(op.toArray(OWLClassExpression[]::new));
	}
