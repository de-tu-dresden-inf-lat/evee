...
import de.tu_dresden.inf.lat.proofs.data.AbstractSimpleOWLProofGenerator
import de.tu_dresden.inf.lat.proofs.interfaces.{IProof, ISimpleProofGenerator, IProofWriter}
...
object GenerateAllProofs {
  ...
  def printUsageAndExit() = {
    println("Usage: ")
    println("de.tu_dresden.inf.lat.forgettingBasedProofs.GenerateALLProofs ONTOLOGY_PATH")
    System.exit(1)
  }
  ...
  def main(args: Array[String]) = {
    ...
    val forgettingBasedProofs = new GenerateAllProofs(proofGenerator)
    ...
    if(new File(lastFile).exists()){
      ...
      try{
        ...
        forgettingBasedProofs.continueAt = Some(lastSubsumption)
      } finally
        resource.close()
    }
    ...
    forgettingBasedProofs.generateProofs(ontology, proofWriter, proofsDir+"/")
  }
